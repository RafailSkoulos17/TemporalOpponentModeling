...
def training_data(type):
    for root, dirs, files in os.walk("training_logs_updated"):
            ...
            if str(filename).count(type) == 2:  # if we 2 have agents from the wanted class
                ...
                for bid in bids:
                    ...
                    if "move1" in bid:
                        ...
                        temp_util1.append(bid["diff1"])
                    if "move2" in bid:
                        temp2.append([bid["move2"][1]])
                        temp_util2.append(bid["diff2"])
        ...
        with open(type + "_naive_utils.txt", "wb") as fp:
            pickle.dump(train_naive_utils, fp)
...
def testing_data():
    for root, dirs, files in os.walk("test_logs_updated"):
        ...
        with open("test_naive_utils.txt", "wb") as fp:
            pickle.dump(test_naive_utils, fp)
...
with open("conceder_naive_utils.txt", "rb") as fp:
    t = pickle.load(fp)
for tt in t:
    print(tt)
print(len(t))
